...
interface GestureMapControllerConfig {
  map: google.maps.Map;             // Google Maps instance (required)
...
| --- | --- | --- | --- |
| `map` | `google.maps.Map` | Yes | The Google Maps instance to control. |
...
Lower-level class for advanced use cases where you want to manage the gesture pipeline yourself and only use the Google Maps interaction layer.

### Constructor

```ts
new GoogleMapsGestureInteraction(map: google.maps.Map)
```

Creates a gesture interaction bound to the given Google Maps instance. Does not start any webcam or detection. You are responsible for providing `StateMachineOutput` frames.

### Methods

| Method | Returns | Description |
| --- | --- | --- |
| `apply(output: StateMachineOutput)` | `void` | Applies a single `StateMachineOutput` frame to the map. Call this each time the state machine produces output (typically once per video frame). Internally applies pan, zoom, and heading changes using the Google Maps API. |

This class is useful when you want to wire up your own `GestureController` and `GestureStateMachine` from `@map-gesture-controls/core` and connect them to a Google Maps instance without using the higher-level `GestureMapController`.
...
```ts
// All of these work from the google-maps package:
import {
  GestureController,
  GestureStateMachine,
  WebcamOverlay,
  ...
  LANDMARKS,
  COLORS,
} from '@map-gesture-controls/google-maps';
```
